...
import org.atlas.apps.localization.filters.Filter;
...
public class KalmanFilter extends Filter {
	 ...
	 */
	private Position priorEstimate, estimate, rawValue;
	/**.
	 ...
	 */
	@Override
	public Position applyFilter(Position lastPosition) {
		...
		estimate = new Position(priorEstimate.getX() + (kalmanGain * (rawValue.getX() - priorEstimate.getX())), priorEstimate.getY() + (kalmanGain * (rawValue.getY() - priorEstimate.getY())));
		...
		lastPosition = new Position(estimate.getX(), 	estimate.getY());
